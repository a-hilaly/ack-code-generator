...
func (f *Field) GetReferenceFieldName() (names.Names, error) {
	if f.Names.Original == "" {
		crdName := ""
		if f.CRD != nil {
			crdName = f.CRD.Names.Original
		}
		return names.Names{}, fmt.Errorf(
			"field with empty name inside crd %s", crdName,
		)
	}
...
func (f *Field) ReferenceFieldPath() (string, error) {
	refName, err := f.GetReferenceFieldName()
	if err != nil {
		return "", err
	}
	fieldPathPrefix := strings.TrimSuffix(f.Path, f.Names.Camel)
	return fmt.Sprintf("%s%s", fieldPathPrefix, refName.Camel), nil
}
			...
			for _, memberName := range containerShape.MemberNames() {
				...
				if _, ok := nestedParentFields[memberShape.ShapeName]; ok {
					return nil, fmt.Errorf(
						"detected a cyclic type reference in %s — add the corresponding path to ignore.field_paths in generator.yaml",
