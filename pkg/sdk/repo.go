...
func EnsureRepo(
	...
	if _, err = os.Stat(sdkDir); os.IsNotExist(err) {
		...
		fmt.Fprintf(os.Stderr, "  [timing] git clone: %s\n", time.Since(cloneStart))
	} else {
		fmt.Fprintf(os.Stderr, "  [timing] git clone: skipped (cached)\n")
	}
	...
	if fetchTags && !util.HasTag(sdkDir, sdkVersion) {
		...
		fmt.Fprintf(os.Stderr, "  [timing] git fetch tag %s: %s\n", sdkVersion, time.Since(fetchStart))
	} else if fetchTags {
		fmt.Fprintf(os.Stderr, "  [timing] git fetch: skipped (tag %s exists locally)\n", sdkVersion)
	}
	...
	fmt.Fprintf(os.Stderr, "  [timing] git checkout (%s): %s\n", sdkVersion, time.Since(checkoutStart))
	fmt.Fprintf(os.Stderr, "  [timing] EnsureRepo total: %s\n", time.Since(totalStart))
	return sdkDir, nil
}
