...
func LoadAPI(modelPath string) (map[string]*awssdkmodel.API, error) {
	...
	fmt.Fprintf(os.Stderr, "    [timing] read model JSON (%d KB): %s\n", len(file)/1024, time.Since(readStart))
	...
	fmt.Fprintf(os.Stderr, "    [timing] JSON unmarshal (%d shapes): %s\n", len(customAPI.Shapes), time.Since(unmarshalStart))
	...
	fmt.Fprintf(os.Stderr, "    [timing] buildAPI (v2->v1 conversion): %s\n", time.Since(buildStart))
	...
	fmt.Fprintf(os.Stderr, "    [timing] API.Setup(): %s\n", time.Since(setupStart))

	cleanUpBadDefaultValueAssignment(newApi)

	fmt.Fprintf(os.Stderr, "    [timing] LoadAPI total: %s\n", time.Since(totalStart))
